...
The Rust version of the [mock-lab](../../README.md) proxymock demo. It serves an HTTP API on
...
```shell
proxymock record -- cargo run                    # 1. record the downstream calls
../../lab/tests/run_tests.sh --recording            # 2. second terminal: drive every endpoint
proxymock web                                    # 3. browse the recorded traffic (:7788)
proxymock mock -- cargo run                       # 4. serve the downstream from the recording
proxymock replay --test-against http://localhost:8080   # 5. replay (or use Replay in proxymock web)
```

`proxymock record` exports the proxy and TLS settings. `reqwest` picks up `HTTP_PROXY` /
`HTTPS_PROXY` automatically, but rustls does **not** read `SSL_CERT_FILE`. The app loads
that file as an extra root CA so proxymock's TLS interception verifies. Without that, the
downstream HTTPS call through proxymock fails verification.
