...
// every LogQL query uses the file values unchanged.
package main
...
func main() {
	...
	// Extra seconds past the ceiling leave room for Alloy to tail and push the
	// last lines of the interval into Loki.
	queryEnd := end.UTC().Truncate(time.Second).Add(5 * time.Second)
