...
- **Blueprint anchoring.** Two locations load, and the script accepts either: a
  `blueprints/` inside `--in` (replay reads `--in` recursively — this one loaded
  in every layout measured), and the workspace's own `blueprints/` beside the
  ...
  not) but is not the gate here: on failure it aborts before
